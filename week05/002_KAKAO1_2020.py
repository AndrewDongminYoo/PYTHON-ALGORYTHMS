...
def string_compression(string):
    ...
    for split_size in range(1, length//2+1):
        compressed_string = ""
        spliced = [string[i:i + split_size] for i in range(0, length, split_size)]
        print(spliced)

        count = 1
        for j in range(1, len(spliced)):
            prev, curr = spliced[j - 1], spliced[j]
            if prev == curr:
                count += 1
            else:
                if count > 1:
        ...
        if count > 1:
            compressed_string += (str(count) + spliced[-1])
        else:
            compressed_string += spliced[-1]
        print(compressed_string)
        answer_array.append(compressed_string)
    return min(answer_array)


word = "abcabcabcabcdededededede"
print("string_compressed:", string_compression(word))  # 14 가 출력되어야 합니다!
